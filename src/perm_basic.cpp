...
DataFrame perm_do(NumericVector sample_0,
                  int decimals,
                  double reps,
                  double tolerance)  {
  ...
  for(int i = 0; i < reps; ++i)  {
    ...
    //double out_av = average_fre(shuffled_num);

    //double out_av = sum(shuffled_num * shuffled_num) / n;
  ...
  double d_perm_p = (total_actual + 1) / (reps + 1);
  double d_av_fre_p = (av_fre_draw + 1) / (reps + 1);
  double d_chi_p = (chi_draw + 1) / (reps + 1);
  double d_g2_p = (g2_draw + 1) / (reps + 1);
  double d_ft_p = (ft_draw + 1) / (reps + 1);
  double d_rms_p = (rms_draw + 1) / (reps + 1);
